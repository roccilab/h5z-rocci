...
template<class T>
CustomHuffmanEncoder<T>::CustomHuffmanEncoder() {
    int x = 1;
    char *y = (char *) &x;
    sysEndianType = (*y == 1) ? 0 : 1;
}
...
template<class T>
typename CustomHuffmanEncoder<T>::HuffmanTree* CustomHuffmanEncoder<T>::createHuffmanTree(int stateNum) {
...
template<class T>
unsigned char CustomHuffmanEncoder<T>::get_cout_for_state(T bin_index) {
    int state = bin_index - offset;
    return (huffmanTree->cout[state]);
}

template<class T>
void CustomHuffmanEncoder<T>::preprocess_encode(const std::vector<T> &bins, int stateNum) {
    preprocess_encode(bins.data(), bins.size(), stateNum);
}

template<class T>
void CustomHuffmanEncoder<T>::preprocess_encode(const T *bins, size_t num_bin, int stateNum) {
    nodeCount = 0;
    if (num_bin == 0) {
...
template<class T>
void CustomHuffmanEncoder<T>::save(uchar *&c) {
    auto cc = c;
    write(offset, c);
...
template<class T>
size_t CustomHuffmanEncoder<T>::size_est() {
...
template<class T>
size_t CustomHuffmanEncoder<T>::encode(const std::vector<T> &bins, uchar *&bytes) {
    return encode(bins.data(), bins.size(), bytes);
}

template<class T>
size_t CustomHuffmanEncoder<T>::encode(const T *bins, size_t num_bin, uchar *&bytes) {
    size_t outSize = 0;
    size_t i = 0;
...
template<class T>
void CustomHuffmanEncoder<T>::postprocess_encode() {
    SZ_FreeHuffman();
}

template<class T>
void CustomHuffmanEncoder<T>::preprocess_decode() {}

template<class T>
std::vector<T> CustomHuffmanEncoder<T>::decode(const uchar *&bytes, size_t targetLength) {
    node t = treeRoot;
...
template<class T>
void CustomHuffmanEncoder<T>::postprocess_decode() {
    SZ_FreeHuffman();
}

template<class T>
void CustomHuffmanEncoder<T>::load(const uchar *&c, size_t &remaining_length) {
    ...
    if (nodeCount <= 256)
        ...
        encodeStartIndex = 1 + 2 * nodeCount * sizeof(unsigned short) + nodeCount * sizeof(unsigned char) + nodeCount * sizeof(T);
    else
...
template<class T>
bool CustomHuffmanEncoder<T>::isLoaded() {
    return loaded;
}
...
template<class T>
typename CustomHuffmanEncoder<T>::node CustomHuffmanEncoder<T>::reconstruct_HuffTree_from_bytes_anyStates(const unsigned char *bytes, uint nodeCount) {
            if (nodeCount <= 256) {
...
template<class T>
template<class T1>
unsigned int CustomHuffmanEncoder<T>::convert_HuffTree_to_bytes_anyStates(unsigned int nodeCount, unsigned char *out) {
...
template<class T>
typename CustomHuffmanEncoder<T>::node CustomHuffmanEncoder<T>::new_node(size_t freq, T c, node a, node b) {
...
template<class T>
typename CustomHuffmanEncoder<T>::node CustomHuffmanEncoder<T>::new_node2(T c, unsigned char t) {
...
template<class T>
void CustomHuffmanEncoder<T>::qinsert(node n) {
...
template<class T>
typename CustomHuffmanEncoder<T>::node CustomHuffmanEncoder<T>::qremove() {
    int i, l;
...
template<class T>
void CustomHuffmanEncoder<T>::build_code(node n, int len, uint64_t out1, uint64_t out2) {
    if (n->t) {
...
template<class T>
void CustomHuffmanEncoder<T>::init(const T *s, size_t length) {
    T max = s[0];
...
template<class T>
template<class T1>
void CustomHuffmanEncoder<T>::pad_tree(T1 *L, T1 *R, T *C, unsigned char *t, unsigned int i, node root) {
            C[i] = root->c;
            t[i] = root->t;
...
template<class T>
template<class T1>
void CustomHuffmanEncoder<T>::unpad_tree(T1 *L, T1 *R, T *C, unsigned char *t, unsigned int i, node root) {
            //root->c = C[i];
            if (root->t == 0) {
                T1 l, r;
                l = L[i];
...
template<class T>
void CustomHuffmanEncoder<T>::SZ_FreeHuffman() {
            if (huffmanTree != NULL) {
                size_t i;
...
template class SZ3::CustomHuffmanEncoder<int>;