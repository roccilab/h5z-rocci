...
size_t blocksizes[] = {64};
bool skip_setting[] = {true};
double sample_rates[] = {0.10};

template<class T, uint N>
std::vector<double> estimate_compress(Config conf, T *data, double abs){
...
int main(int argc, char *argv[]) {
	...
	for(int i = 0; i < files.size(); i++){
		...
		for(int j = 0; j < eb.size(); j++){
			...
			if (dim == 1) {
				Config config(r1);
				results = estimate_compress<float, 1>(config, data_, eb_);
			} else if (dim == 2) {
				Config config(r2, r1);
				results = estimate_compress<float, 2>(config, data_, eb_);
			} else if (dim == 3) {
				Config config(r3, r2, r1);
				results = estimate_compress<float, 3>(config, data_, eb_);
			}
